...
|----------|-----------|-------|
| **`learn/`** | Teaching the agent ONE primitive (or one adapter) in isolation. Each file demonstrates a single concept. | `prompt`, `dynamic_chain`, `dynamic_tree`, `conditional`, `loop`, `reflector`, `hello`, `cyberdiner_hello`, `surrealdb_roundtrip` |
